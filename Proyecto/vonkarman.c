...
int maxlevel = 9;
face vector muv[];
const double end_time = 30.0;   //Tiempo de finalización de la simulación, en este caso 30
...
//Checkeo el número de iteraciones para trackear el progreso de la simulación:
event logfile (i++)
  fprintf (stderr, "%d %g %d %d\n", i, t, mgp.i, mgu.i);


//Produzco algunas animaciones iniciales de la simulacion:
event movies (i += 1; t <= end_time) //Genera un output todas las iteraciones, hasta llegar al end_time
{
  scalar omega[], m[];
  vorticity (u, omega);
  foreach()
    m[] = cs[] - 0.5;
  output_ppm (omega, file = "vort_improved.mp4", 
    box = {{-0.5,-0.5},{7.5,0.5}},
    n = 2560,
	min = -10, max = 10, 
    linear = true,
    mask = m,
    opt = "crf=18");
  output_ppm (f, file = "f_improved.mp4", 
    box = {{-0.5,-0.5},{7.5,0.5}},
    n = 2560,
    linear = true,
    min = 0, max = 1,
...
//Fijo los margenes de error para el modelamiento y sus componentes:
event adapt (i++) {
  adapt_wavelet ({cs,u,f}, {1e-2,3e-2,3e-2,3e-2}, maxlevel, 4);
}

event end(t = end_time) {
    return 1;   //Detiene la simulación al llegar al tiempo antes establecido
}